...
def fig_e5():
    ...
    fig.subplots_adjust(top=0.84, bottom=0.11, left=0.13, right=0.97)
    ...
    for m in MODEL_ORDER:
        ...
        delta = mp - mi
        ax.annotate(f"  {MODEL_LABELS[m]}  Δ = {delta:+.0%}",
                    xy=(x_phr, mp), xytext=(8, 0), textcoords="offset points",
    ...
    fig.text(0.13, 0.94,
             "Virtue-ethics framing flips defection across all four models",
             fontsize=11.5, fontweight="semibold", ha="left")
    fig.text(0.13, 0.895,
             "Within-model shift from integrity to phronesis framing  ·  3 seeds × 20 rounds vs AllD",
...
def fig_f1():
    ...
    with open(CSV_DIR / "trajectory_metrics.csv") as f:
        for row in csv.DictReader(f):
            ...
            if row["persona"] not in PERSONA_ORDER:
                continue
            try:
                cells[(row["model"], row["persona"])].append(float(row["D_raw"]))
            except ValueError:
                pass

    fig, ax = plt.subplots(figsize=(7.4, 4.8))
    fig.subplots_adjust(top=0.84, bottom=0.13, left=0.10, right=0.78)

    n_p, n_m = len(PERSONA_ORDER), len(MODEL_ORDER)
    bar_w = 0.78 / n_m
    for i, persona in enumerate(PERSONA_ORDER):
        for j, model in enumerate(MODEL_ORDER):
            xs = cells.get((model, persona), [])
            if not xs:
                continue
            m, se, _ = mean_se(xs)
            x = i + (j - (n_m - 1) / 2) * bar_w
    ...
    ax.legend(handles=handles, loc="center left", bbox_to_anchor=(1.01, 0.5),
              fontsize=9, handletextpad=0.4, borderaxespad=0)
...
def fig_f2():
    ...
    fig.subplots_adjust(top=0.84, bottom=0.13, left=0.11, right=0.79)
    ...
    handles = [ax.plot([], [], color=MODEL_COLORS[m], linewidth=1.8,
                       marker="o", markersize=7, markeredgecolor="white",
                       markeredgewidth=1.2, label=MODEL_LABELS[m])[0]
               for m in MODEL_ORDER]
    ax.legend(handles=handles, loc="center left", bbox_to_anchor=(1.01, 0.5),
              fontsize=9, handletextpad=0.6, borderaxespad=0)
...
def fig_f3():
    ...
    fig.subplots_adjust(top=0.84, bottom=0.13, left=0.10, right=0.80)
    ...
    handles = [ax.plot([], [], color=MODEL_COLORS[m], linewidth=2.0,
                       label=MODEL_LABELS[m])[0] for m in MODEL_ORDER]
    ax.legend(handles=handles, loc="center left", bbox_to_anchor=(1.01, 0.5),
              fontsize=9, handletextpad=0.8, borderaxespad=0)
...
def fig_f4():
        ...
        for model in MODEL_ORDER:
            for var_key, _ in variants:
                ...
                for fp in sorted(d.glob("seed*_oppAllD.jsonl")):
                    s = summary_of(fp)
                    if s is not None:
                        cells[(model, var_key)].append(s["raw_defection_rate"])
    ...
    if e4.exists():
        for fp in sorted(e4.rglob("deontologist/seed*_oppAllD.jsonl")):
            ...
            s = summary_of(fp)
            if s is not None:
                cells[(model, "deontologist_original")].append(s["raw_defection_rate"])

    fig, ax = plt.subplots(figsize=(7.6, 4.8))
    fig.subplots_adjust(top=0.84, bottom=0.16, left=0.10, right=0.78)

    n_v, n_m = len(variants), len(MODEL_ORDER)
    bar_w = 0.78 / n_m
    for i, (var_key, _) in enumerate(variants):
        for j, model in enumerate(MODEL_ORDER):
            xs = cells.get((model, var_key), [])
            if not xs:
                continue
            m, se, _ = mean_se(xs)
            x = i + (j - (n_m - 1) / 2) * bar_w
    ...
    header(fig,
           "Paraphrase sensitivity: GPT-4o mini swings ~70 pts; flagships hold under 25 pts",
...
def fig_f5():
    ...
    model_handles = [ax.scatter([], [], color=MODEL_COLORS[m], s=42,
...
def fig_summary():
    ...
    gs = gridspec.GridSpec(2, 2, figure=fig,
                           left=0.07, right=0.985, top=0.86, bottom=0.08,
    ...
    axA.set_title("Effect generalizes across families, generations, and tiers",
                  loc="left", fontsize=10.5, fontweight="semibold", pad=4)
    panel_letter(axA, "a")
    ...
    axB.set_title("When the moral persona finally breaks",
                  loc="left", fontsize=10.5, fontweight="semibold", pad=4)
    panel_letter(axB, "b")

    model_handles = [axB.scatter([], [], color=MODEL_COLORS[m], s=46,
                                  edgecolor="white", linewidth=1.0,
                                  label=MODEL_LABELS[m]) for m in MODEL_ORDER]
    axB.legend(handles=model_handles, loc="upper left", fontsize=7.5,
    ...
    axC.set_title("Behaviour and language come apart: hypocrisy scales with defection",
                  loc="left", fontsize=10.5, fontweight="semibold", pad=4)
    panel_letter(axC, "c")
    ...
    axD.set_title("Cheap models defect more under every moral persona",
                  loc="left", fontsize=10.5, fontweight="semibold", pad=4)
    panel_letter(axD, "d")

    fig.text(0.07, 0.955,
             "Moral framing — not the framework label — controls defection in LLM agents",
             fontsize=15, fontweight="semibold", ha="left")
    fig.text(0.07, 0.920,
             "Defection (D) vs an always-defecting opponent in an iterated prisoner's dilemma  ·  "
             "20 rounds, 3 seeds per cell  ·  mean ± 1 SE; faint dots = seeds",
